...
%SC: upstream change, maybe not appropriate
\newpage
...
\verb.\index{ecole@école}.     &école, 4& clef accentuée
\end{tabular}
\end{center}
\end{table}
...
\item Vous pouvez utiliser l'extension \pai{aeguill} (\emph{Almost
  ...
  préambule la ligne\newline
  \verb+\usepackage{aeguill}+\paih{aeguill} pour
...
Si votre document est codé en unicode et que vous utilisez l'option
...
\begin{code}
\begin{verbatim}
\RequirePackage{ifpdf} % utilisation de pdfTeX?
\ifpdf
...
exemple), le programme \TeX{} par défaut est en fait pdf\TeX{}. Il
s'adaptera automatiquement à la production de pdf ou de dvi en
fonction des réglages de votre classe de document. Si vous utilisez le
code ci-dessus, alors vous pouvez toujours utiliser la commande
\verb|pdflatex| pour obtenir une sortie pdf et \verb|latex| pour une
sortie dvi.
