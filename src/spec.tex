...
\begin{description}
\item[\texttt{screen}] pour une présentation à l'écran. Utilisez
...
\item[\texttt{panelright}] ajoute un panneau de navigation à droite de
  ...
  \texttt{panelleft}. Si vous voulez vous passer de panneaux utilisez
  \texttt{nopanel} ;
\item[\texttt{french}] ou tout autre langage pris en compte modifiera
  ...
  option est indépendante des options de l'extension babel. Si votre
  langage n'est pas géré par \pai{pdfscreen}, vous pouvez tout de même
  localiser le panneau via le fichier \texttt{pdfscreen.cfg}, voyez
  \texttt{pdfscreen.cfg..specimen} pour un exemple ;
...
\item[\ci{screensize}\textmd{\texttt{\{}\emph{largeur}\texttt{\}\{}\emph{hauteur}\texttt{\}}}]
...
\end{lscommand}
\noindent ou si vous préférez un fond uni, vous pouvez définir sa
couleur avec
\begin{lscommand}
...
mendier sa pitance. En ligne de commande, cela ressemble à :
%SC: sing and dance and beg for food. Need better translation
\begin{lscommand}
\verb+ppower4 xy.pdf xyz.pdf+
\end{lscommand}

Pour maîtriser le contenu d'un seul transparent, utilisez
...
Une compilation de l'exemple plus haut s'achèvera par un message
d'erreur :
\begin{verbatim}
