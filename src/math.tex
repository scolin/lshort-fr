...
référencer depuis un autre endroit via les commandes \ci{ref} ou
\ci{eqref} de l'extension \pai{amsmath} :
\begin{example}
...
comme pour les indices et les exposants.\index{exposant}
...
\end{eqnarray}
\end{example}
On peut considérer qu'il y a trop d'espace de part et d'autre de la
colonne centrale, autour des signes d'égalité.  Il peut être réduit
par la commande \verb|\setlength\arraycolsep{2pt}| comme dans
l'exemple suivant.
...
\verb|\left[  \right]|. Les versions plus grandes des crochets sont
produits ici par les commandes \ci{biggl} and \ci{biggr} pour les
crochets gauche et droit, respectivement.


% Nouvelle section (df)
%SC: new section to backport to lshort or suppress
...
L'extension \pai{amsthm} met à disposition la commande
