...
%SC: just to avoid bad emacs colorizing $
...
\verb|\end{equation}|.\footnote{Il s'agit d'une commande de
...
Vous pourrez d'autres sortes de flèches comme $\hookrightarrow$ et
...
\footnote{L'environnement \ei{align} provient
  ...
  \texttt{amsmat} est \ei{eqnarray}, mais il est déconseillé de
...
\footnote{Si vous avez de nombreuses constructions de ce genre,
  essayer l'environnement \ei{cases} d'\texttt{amsmath} peut en valoir
...
\section{Espacement en mode math<E9>matique}
...
L'extension \pai{amsthm} (qui fait partie d'\AmS-LaTeX) met à disposition la commande
