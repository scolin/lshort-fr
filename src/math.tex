...
%MPG: correction à faire remonter upstream
...
version étoilée d'\texttt{equation}, \ei{equation*}\footnote{Fournie par
  ...
  charger \package{amsmath}.}, ou mieux encore,
...
et \ci{\iffalse[\fi]}. Aussi\footnote{En fait, \ci{[\iffalse]\fi} et
  \ci{\iffalse[\fi]} peuvent être parfaitement lisibles si vous prenez la pein
  de les laisser seuls sur une ligne. Un autre avantage d'\ei{equation*} est
  qu'il est facile d'activer la numérotation : il suffit d'enlever les
  étoiles.} nous recommandons d'utiliser plutôt \ei{equation} et
\ei{equation*}.
...
dessus\footnote{Parfois appellés \emph{limites}. \NdT}. Cette commande ne peut
être utilisée
...
\label{sec:multline}
\index{equation!longue@équation}
...
Bien que ce soit la solution la plus facile, on peut souvent obtenir un
meilleur résultat avec l'environnement \ei{IEEEeqnarray} (voir
...
Dans le cas le plus général, considérons une suite d'égalités qui ne tiennent
...
Pour grouper ensemble plusieurs équations, on peut aussi utiliser
l'environnement \ei{align}\footnote{L'environnement \texttt{align} peut aussi
  ...
  cette usage plutôt rare, nous recommandons également d'utiliser
  l'environnement \ei{IEEEeqnarray} avec un argument comme, par exemple
...
Cependant, cette approche pose problème dès qu'un ligne est trop longue.
\begin{example}
\begin{align}
  a & = b + c \\
  & = d + e + f + g + h + i 
...
signe d'égalité. Bioen sûr, il est possible d'ajouter un peu d'espace avec,
par exemple \verb+\hspace{...}+, mais il est difficile d'obtenir un alignement
...
\begin{itemize}
  \item Les espaces autour du signe d'égalité sont trop grands. En particulier,
    ce ne sont pas les mêmes qu'avec les environnement \ei{multline} ou
    \ei{equation}.
\begin{example}
\begin{eqnarray}
  a & = & a = a
...
%MPG: fixed typo \texttt -> \verb
En fait, on peut spécifier n'importe quel nombre de colonnes. Par exemple,
\verb+{c}+ donnera une seule colonne dans laquelle toutes les lignes seront
centrées, ou bien \verb+{rCll}+ donnera une quatrième colonne aligné à gauche,
par exemple pour des commentaires. Mieux, en plus de \texttt{l}, \texttt{c},
\texttt{r}, \texttt{L}, \texttt{C}, \texttt{R} pour les entrées en mode math,
...
l'environnement \texttt{taular} : le premier argument \verb+3+ dit qu'il faut
...
situations, comme par exemple devant
  \begin{itemize}
    ...
    \ci{max}, etc. ;
  \item une intégrale \ci{int} ou une somme sum \ci{sum} ;
...
\item Pour supprimer la numérotation sur une ligne particulière, on dispose de
  ...
  la ligne concernée. En plus d'améliorer la lisibilité du code source, ceci
...
\item Il existe également une version étoilée de l'environnement, qui supprime
  ...
  d'équation sur une ligne particulière avec la commande \ci{IEEEyesnumber}.
\begin{example}
\begin{IEEEeqnarray*}{rCl}
  a & = & b + c \\
  & = & d + e \IEEEyesnumber\\
...
\item On peut également obtenir des numéros de sous-équation avec
  \ci{IEEEyessubnumber}.
\begin{example}
\begin{IEEEeqnarray}{rCl}
  a & = & b + c
...
\ci{theoremstyle}\verb|{|\emph{style}\verb|}| permet de choisir un type de
...
L'environnement \og decret \fg{} utilise le même compteur que l'environnment
...
Malheureusement, cette correction ne fonctionne pas pour \ei{IEEEeqnarray}.
\begin{example}
\begin{proof}
...
systématiquement une colonnes invisible de chaque côté de l'équation, qui
...
automatiquement par \texttt{IEEEeqnarray}, et un à droite de l'équation. Mais
...
espace \verb=+= dont on ne veut pas ici.
...
$\Box$ est bien plus proche de l'équation que dans le première.
