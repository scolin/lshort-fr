...
bien que cela requière quelques efforts mathématiques. Si en plus un
langage de programmation externe est utilisé pour générer des blocs
...
Des extensions comme \pai{epic} et \pai{eepic} (décrits dans
...
écrits avec des objectifs précis en tête. \graphicscompanion{} décrit en
détail une grande variété de ces environnements (à ne pas confondre avec
\companion).
...
avec la même interface. L'extension PGF fournit
...
%MPG: déplacé ici pour lutter contre la page sous-remplie.
Si vous être programmeur, vous remarquerez l'usage, familier, du point-virgule
\og \texttt{;} \fg pour séparer les instructions.  Avec 
\ci{usetikzlibrary} dans le préambule, vous pouvez activer de nombreuses
fonctionnalités pour le dessin de formes spéciales.
...
Par exemple, la bibliothèque \texttt{decorations.pathmorphing}
permet d'obtenir des boîtes légérement tordues.
...
montrer que son résultat. Si vous lisez la documentation de PGF, vous
