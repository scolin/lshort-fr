...
\ci{qbezier}, le «\texttt{q}» signifiant «quadratique». De nombreuses
...
%SC: TODO: typo dans la version anglaise (ontrary)
...
\LaTeX{} standard et fonctionne très bien pour des tâches simple ou si
vous souhaitez plus de contrôle sur la position exacte des éléments
sur une page. Mais pour tout labeur graphique complexe, vous devriez
...
PGF est accompagnée par un langage de haut niveau fourni par
...
Comme mentionné plus haut, il existe un très bon manuel pour PGF et
...
\end{tikzpicture}
\end{example}
%SC: redéplacement de ce paragraphe à sa place initiale.
...
caractères utilisés par le langage TikZ soient modifiés par babel, ce
qui conduit à des erreurs étranges. Pour éviter ce problème, ajoutez
la commande \ci{shorthandoff} à votre code.

Remarquez les boucles «pour» (\texttt{foreach}) dans l'example suivant~:
\begin{example}
