...
\section{Multilingual Texts with Polyglossia}
...
generated text strings in your non-english documents. The option \verb|babelshorthands| enables
...
Note that some \hologo{TeX} command are ligatures
...
\end{verbatim}
\end{lined}
\caption[All in one preamble]{All in one preamble that takes care of all three pdf generating \TeX{} engines.} \label{allinone}
\end{figure}
...
preamble. This example uses the \pai{csquote} package which generates
...
\begin{lscommand}
\verb|English text. \textgerman{Gesundheit} is actually a German word.|
\end{lscommand}
