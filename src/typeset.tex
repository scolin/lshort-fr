...
% $Id: typeset.tex 449 2010-12-14 16:53:51Z oetiker $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
critères. Dans ce cas le message \og \wiat{\texttt{underfull \bs hbox}}
...
(c'est-à-dire \emph{langueB}). Vous pouvez utiliser la commande
\begin{lscommand}
...
Depuis le début de ce siècle la majorité des systèmes d'exploitation
sont basés sur Unicode (par exemple, Windows XP, MacOS X). C'est
pourquoi il est recommandé d'utiliser \iei{utf8} pour tout nouveau
projet. Le codage \iei{utf8} utilisé par \pai{inputenc} n'est défini que
pour les caractères effectivement fournis par les polices
utilisées. Si vous avez besoin de plus de caractères (non-latins),
...
trouve chaque caractère. Plusieurs codages d'entrée sont susceptibles d'être
traduits en un codage de fonte donné, ce qui réduit le nombre
de dispositions de polices requises. Les codages de polices sont manipulés
...
visuellement correct, cette approche empêche le système de césure de
...
ponctuation pour la plupart des langues européennes qui utilisent un
...
préférable d'utiliser utf8 sur un système moderne.
...
\item Nous devons être capables d'éditer des \wi{fichiers d'entrée en
  ...
  sorte qu'éditer un fichier texte coréen n'est plus autant un
  problème, même sur un système non-coréen.

\item \TeX{} et \LaTeX{} ont été conçus à l'origine pour des
...
par Werner~Lemberg.\footnote{%
  ...
  H\LaTeX{} et h\LaTeX{}p sont spécifiques au coréen et fournissent une
...
\item L'idée principale derrière l'utilisation d'un système
...
qui précéde est une consonne ou une voyelle (la réalité est plus
complexe, mais c'est l'idée générale). Les coréens natifs n'ont
...
\end{lscommand}
\noindent où \iei{mn} correspond à l'encodage d'entrée \iei{cp1251}.
Si vous préférez la modernité, utilisez \iei{utf8} à la place.
...
langages dans le même document, en particulier si ces langages ne sont
pas basés sur un alphabet latin. Il existe deux systèmes \hologo{TeX}
capables de prendre de l'Unicode directement en entrée :

\begin{description}
\item[\hologo{XeTeX}], développé initialement pour MacOS X mais
  désormais disponible pour toutes les architectures. Il fut inclus à
  partir de 2007 dans TeXLive.
\item[\hologo{LuaTeX}], le successeur de pdf\TeX. Il fut inclus à
  partir de 2008 dans TeXLive.
\end{description}

Nous décrivons dans la suite \hologo{XeLaTeX} tel qu'il existe dans
TeXLive 2010.

\subsubsection{Démarrage rapide}

Convertir un fichier \LaTeX\ existant vers un format compatible avec
\hologo{XeLaTeX} entraîne la procédure suivante :
...
L'extension \pai{polyglossia}\cite{polyglossia} est un substitut à
\pai{babel}. Elle prend soin des motifs de césure et des chaînes de
textes générées automatiquement. L'option \verb|babelshorthands|
...
est Latin Modern Roman. Il est à noter ici un fait peu connu, à savoir
que certaines commandes \hologo{TeX} sont des ligatures définies dans
les polices Computer Modern. Si vous souhaitez les utiliser avec une
police non-\hologo{TeX}, vous devez les émuler. L'option
...
Unicode, pour pouvez simplement\footnote{Pour des valeurs suffisamment
  petites de \og simple \fg.} entrer les caractères dans votre éditeur
...
%MPG: oui, parce que là ça vient après \clearpage, ça fait bizare ÀMHA.
En section~\ref{eps}, vous apprendrez à inclure des figures
