...
avec la commande : 
\begin{lscommand}
\ci{usepackage}\verb|[|\emph{langue},\emph{langue}\verb|]{babel}| 
\end{lscommand}
...
Si vous appelez babel avec plusieurs langues, vous pouvez utiliser la
commande
\begin{lscommand}
\ci{selectlanguage}\verb|{|\emph{langue}\verb|}|
\end{lscommand}
pour définir la langue courante.
...
Cependant, il n'y a malheureusement pas de version Postscript de la
police EC, ce qui causera des problèmes lors de la génération de
versions PDF de vos documents. Si cela vous pose effectivement
problème, vous pouvez essayer d'utiliser le package \pai{ae}
(\emph{Almost European Computer Modern}) qui utilise les caractères de
la police Computer Modern d'origine, mais les réarrange dans le même
ordre que ceux de la police EC.
...
Il est cependant nécessaire d'activer les accents utilisés en
portugais avec les commandes suivantes :
\begin{lscommand}
...
Vous pouvez désormais utiliser normalement tous les accents de la
langue portugaise. Voyez la table~\ref{portuguese} qui récapitule ce
qu'il y a à ajouter en préambule pour écrire en langue portugaise.

\begin{table}[!hbp]
\caption{Préambule pour écrire en portugais.} \label{portuguese}
\begin{lined}{5cm}
\begin{verbatim}
...
Pour le français, les options \pai{francais} ou \pai{frenchb}
(équivalentes, pour des raisons historiques, mais \emph{pas} \pai{french})
de l'extension \pai{babel}
appliquent les règles de césure spécifiques du français et adaptent
...
\ci{chapter}\verb|{...}| est également reconnue, (elle s'intercale
...
\index{"|@ \verb."|.} 
L'argument \emph{position} définit la position verticale de
l'environnement complet, avec \texttt{t}, \texttt{b} and \texttt{c}
pour l'aligner en haut (\emph{top}), en bas (\emph{bottom}) ou au
centre (\emph{center}) respectivement.
