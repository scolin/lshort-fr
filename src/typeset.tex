...
\footnote{
\emph{Différente à tout prix}, traduction du \emph{Different at
...
\end{lscommand}
\noindent suggèrent (en anglais) où un saut de ligne ou de page peut
apparaître ou non. Ces commandes permettent à l'auteur de paramétrer
leur action par l'intermédiaire du paramètre optionnel \emph{n}
pouvant prendre une valeur entre zéro et quatre. En donnant à \emph{n}
...
%SC: suggestion de mpg: \eei au lieu de \wi pour overfull hbox ?
...
L'extension \pai{textcomp} met aussi à disposition le symbole degré
...
français. \emph{NdT.}}.
%SC: à revérifier dans le document produit
\begin{example}
\Large Not shelfful\\
but shelf{}ful
\end{example}
...
\item \LaTeX{} doit connaître les règles de césure de la nouvelle
    ...
    \item Certaines règles typographiques changent en fonction de la
      ...
      impose une espace avant le caractère deux-points (:). Ces changements
...
\end{lscommand}
\noindent après la commande \verb|\documentclass|. La liste des langues intégrées à
...
%SC: danger, la commande package ci-dess(o)us semble être indépendante de
%\pai. Il faudra les réconcilier.
...
\end{lscommand}
\noindent alors la dernière langue dans la liste d'options sera active
...
symboles spécifiques de l'alphabet d'une langue. \LaTeXe{} gère la
variété des codages d'entrée des différents langages et plateformes
grâce à l'extension \pai{inputenc} :
\label{inputenc}
\begin{lscommand}
...
caractères spéciaux. Par exemple, l'umlaut allemand \"a est codé 132
sous OS/2, sous les systèmes Unix utilisant ISO-LATIN~1 il est codé
228 et sous le codage cyrillique cp1251 pour Windows cette lettre
n'existe même pas; c'est pourquoi vous devez exploiter cette
fonctionnalité avec soin.

Pour visualiser correctement votre fichier source sous un autre
éditeur, il faut donc sélectionner l'encodage correspondant à celui
avec lequel le fichier a été créé. La plupart des éditeurs de texte
...
Par contre, votre fichier sera compilable sans problème sur tout
ordinateur, du moment que l'option passée à \package{inputenc}
correspond bien à l'encodage réel du document.

Les encodages suivants peuvent s'avérer utiles en fonction du type de
système sous lequel vous travaillez.
\footnote{Pour en savoir plus sur les codages d'entrée reconnus
...
%SC: à réintégrer correctement dans la 4.26 (+ envoyer upstream)
...
codages d'entrée, vous devriez considérer l'utilisation d'unicode à
...
\end{lscommand}
\noindent vous permettra de créer des fichiers d'entrée \LaTeX{} en
\iei{utf8x}, un codage multi-octets où chaque caractère peut nécessiter
entre un et quatre octets pour être codé.
\footnote{NdT. D'anciennes versions de ce document préconisent la
  conversion des caractères accentués et spéciaux en séquences du type
  {\bs' e} pour l'échange de documents \LaTeX{}. Cette
  pratique tend à être de moins en moins nécessaire grâce à
...
Pour faciliter la lecture, \package{inputenc} est utilisé dans la suite
...
trouve chaque caractère. Des codages multiples pourraient être mis en
correspondance avec un codage unique, ce qui réduit le nombre
d'ensembles de polices requis. Les codages de polices sont manipulés
via l'extension \pai{fontenc}: \label{fontenc}
\begin{lscommand}
\ci{usepackage}\verb|[|\emph{codage}\verb|]{fontenc}| \index{codages des polices}
\end{lscommand}
\noindent où \emph{codage} est le codage de la police. Il est possible
d'en charger plusieurs simultanément.

Le codage de police par défaut de \LaTeX{} est \label{OT1} \fei{OT1},
celui de la police \TeX{} Computer Modern originelle. Celui-ci
contient seulement les 128 caractères de l'ensemble ASCII sur 7
bits. Au besoin, \TeX{} fabrique des caractères accentués en combinant
un caractère normal avec un accent. Bien que le résultat soit
visuellement parfait, cette approche empêche le système de césure de
fonctionner pour les mots contenant des caractères accentués. De plus,
certaines lettres latines ne peuvent être fabriquées de cette façon,
...
proches de CM furent proposées. Les polices \emph{Extended Cork} (EC)
dans le codage \fei{T1} contiennent les lettres et caractères de
ponctuation pour la plupart des langues européennes basées sur un
alphabet latin. L'ensemble de polices LH contient les lettres pour
formatter les langages à l'alphabet cyrillique. À cause du nombre
imposant de glyphes cyrilliques, ils sont organisés en quatre codages
de police : \fei{T2A}, \fei{T2B}, \fei{T2C} et \fei{X2}.
\footnote{La liste des langages pris en compte par chacun de ces
  codages peut être consultée dans \cite{cyrguide}.}
Le paquet CB contient des polices dans le codage \fei{LGR} pour la
composition de textes en grec.

L'usage de ces polices activera ou améliorera les césures dans les
...
poids, formes et tailles optiquement échelonnables de polices.
...
\end{lscommand}
Si vous êtes au Brésil, remplacez le langage par \texttt{\wi{brazilian}}.
...
%SC: suggestion de mpg: TODO: bad coding, booh
\begin{table}[!htbp]
...
mais votre texte est alors limité au codage utilisé.
...
françaises (\flqq guil\-le\-mets\frqq). Les typographes allemands les
...
coréennes. L' extension propose également une \og sélection de
...
De nouvelles commandes pour une saisie simplifiée du grec devient
...
ukrainiens à base de letters cyrilliques.
...
voulez utiliser du cyrillique en mode mathématique, vous devrez
...
défaut : pour les trois langues précitées il s'agit de \fei{T2A}. Les
...
citation et des commandes pour faciliter les césures, voyez le
...
\begin{table}[htb]
  ...
  \begin{tabular}{@{}p{.1\hsize}@{}p{.9\hsize}@{}}
   ...
   \verb|"-| & Un tiret explicit autorisant la césure dans
               le reste du mot.                             \\
   ...
   \verb|"<| & Pour les guillemets à gauche (comme $<\!\!<$).  \\
   \verb|">| & Pour les guillemets à droite (comme $>\!\!>$). \\
   \hline
  \end{tabular}
  \end{center}
\end{table}
...
\item[Synthèse :] cependant,
une liste peut très bien 
résumer des faits.
\end{description}
\end{enumerate}
\end{example}
...
avec un \emph{abstract}, ou résumé, censé donner au lecteur une vue
...
utilisé dans les documents utilisant la classe article.
...
colonne qui fera passer le texte à la ligne comme pour un paragraph
usuel.
...
le faire à l'aide des valeurs \ci{arraystretch} et \ci{tabcolsep}.
...
\wi{montant}.}. Donnez à ce filet \ci{rule} la hauteur voulue.
...
% faudrait demander à upstream
En section~\ref{eps}, vous apprendrez à inclure une figure
...
tous ?). Il faut alors les faire précéder de \ci{protect}.
