...
idée d'ensemble du fonctionnement de \LaTeX{} qui vous permettra de
...
Certaines commandes sont suivies d'un \wi{paramètre} fourni entre
...
source, il ignore le reste de la ligne en cours, le retour à la ligne
et les espaces au début de la ligne suivante.
...
Le caractère \texttt{\%} peut également être utilisé pour couper des
lignes trop longues dans le fichier d'entrée, lorsqu'aucun espace ou
retour à la ligne n'est autorisé.
...
l'intérieur d'autres environnements complexes, tels que le mode
...
\caption[Exemple d'un article de revue plus réaliste.]{Exemple d'un
  ...
  cet exemple vous seront expliquées plus tard en introduction.} \label{document}
\end{figure}
...
soit déjà installé sur votre ordinateur.\footnote{
...
des matières et les références internes soient à jour. S'il y a un
bogue dans votre fichier, \LaTeX{} vous le signalera et s'arrêtera.
...
\item [\normalfont\texttt{proc}] pour des minutes (de conférence, par
  exemple). Cette classe est basée sur la classe article.
  \index{classe proc}
\item [\normalfont\texttt{minimal}] est aussi réduite que
  possible. Elle définit uniquement une taille de page et une police
...
nombre d'extensions préinstallées. Si vous travaillez sur un système
de type Unix, utilisez la commande \texttt{texdoc} pour accéder à la
...
qu'il en manque, n'hésitez pas à contacter les auteurs:
...
\item[\eei{.dvi}] signifie \emph{DeVice Independent}, c'est le fichier
  que l'on visualise à l'écran et qui servira à l'impression (par
