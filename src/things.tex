...
d'une ligne sont en général ignorées et un retour à la ligne unique est
...
\begin{code}
\verb.$ & % # _ { }  ~  ^  \ . %$
%SC: mpg suggère (grâce à l'environnement code ?) :
%  #  $  %  ^  &  _  {  }  ~  \ .
 \end{code}
...
et tous les espaces au début de la ligne\footnote{Ceci est habituel et n'est pas propre à \verb|%|.
  \NdT}  suivante.
...
\ldots{} Et ainsi s'achève mon ouvrage.
\end{document}
\end{verbatim}
\end{lined}
...
Windows avec la distribution MikTeX, vous pouvez essayer via \texttt{yap} (\emph{yet another
  previewer} -- NdT. \emph{encore un autre prévisualiseur}).
...
\item [\normalfont\texttt{minimal}] est aussi réduite que
  ...
  de base. Elle est utilisée principale à des fins de déboguage.
