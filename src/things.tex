...
\ei{comment} environment you have to add the command
...
description assumes that a working \LaTeX{} installation already sits
on your computer.
...
tools for handling \eei{.dvi} files. The first displays them on
screen within the X11 environment and the other
creates a PostScript file for printing. If you are not working on
a Unix system, other means for handling the \eei{.dvi} files may be
provided. 
...
\item[\eei{.aux}] Another file which transports information from one
...
\item[\eei{.ind}] Is the processed .idx file, ready for inclusion into your
  document on the next compile cycle.
\item[\eei{.ilg}] Logfile telling about what \texttt{makeindex} did.
\end{description}
